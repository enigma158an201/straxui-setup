...
mapfile -t tTiersRepos < <(find ${sAptSourcesListSubfolder} -iwholename '*.list') ; sTiersRepos="$(find ${sAptSourcesListSubfolder} -iwholename '*.list')"
bHasSudo=$( { command -v sudo &>/dev/null && echo "true"; } || echo "false")
bHasDoas=$( { command -v doas &>/dev/null && echo "true"; } || echo "false")

getDebianVersion() { sDebMainVersion="$(cat /etc/debian_version)"; echo "${sDebMainVersion%%.*}"; }
...
getNonFreeToNonFreeFirmware() {
	if [[ ${sSourcesListContent} =~ non-free ]] && [[ ! ${sSourcesListContent} =~ non-free-firmware ]]; then
		...
		else 					su - -c "sed -i 's/ non-free/ non-free non-free-firmware /g' ${sAptSourcesListSubfolder}"
		fi
	fi
}
upgradeJessieToStretch() {
	suExecCommandNoPreserveEnv sed -i.old 's/jessie/stretch/g' ${sAptSourcesListFile}
	if [[ "${#sTiersRepos[@]}" -ne 0 ]]; then for sRepo in "${tTiersRepos[@]}"; do suExecCommandNoPreserveEnv sed -i.old 's/jessie/stretch/g' "${sRepo}"; done; fi
...
upgradeStretchToBuster() {
	...
	if [[ "${#sTiersRepos[@]}" -ne 0 ]]; then for sRepo in "${tTiersRepos[@]}"; do suExecCommandNoPreserveEnv sed -i.old 's/stretch/buster/g' "${sRepo}"; done; fi
...
upgradeBusterToBullseye() {
	...
	suExecCommandNoPreserveEnv sed -i.old 's/buster/bullseye/g' ${sAptSourcesListFile} #{,.d/*.list}
	if grep bullseye/updates ${sAptSourcesListFile}; then suExecCommandNoPreserveEnv sed -i 's#/debian-security\ bullseye/updates#\ bullseye-security#g' ${sAptSourcesListFile}; fi
	if [[ "${#sTiersRepos[@]}" -ne 0 ]]; then for sRepo in "${tTiersRepos[@]}"; do suExecCommandNoPreserveEnv sed -i.old 's/buster/bullseye/g' "${sRepo}"; done; fi
}
upgradeBullseyeToBookworm() {
	...
	if [[ "${#sTiersRepos[@]}" -ne 0 ]]; then for sRepo in "${tTiersRepos[@]}"; do suExecCommandNoPreserveEnv sed -i.old 's/bullseye/bookworm/g' "${sRepo}"; done; fi
	getNonFreeToNonFreeFirmware
}
upgradeBookwormToTrixie() {
	suExecCommandNoPreserveEnv sed -i.old 's/bookworm/trixie/g' ${sAptSourcesListFile}
	#suExecCommandNoPreserveEnv sed -i.old 's/non-free/non-free non-free-firmware/g' ${sAptSourcesListFile}
	if [[ "${#sTiersRepos[@]}" -ne 0 ]]; then for sRepo in "${tTiersRepos[@]}"; do suExecCommandNoPreserveEnv sed -i.old 's/bookworm/trixie/g' "${sRepo}"; done; fi
	getNonFreeToNonFreeFirmware
}
upgradeToTesting() {
	...
	else 					su - -c "sed -i 's/bookworm/testing/g' ${sAptSourcesListSubfolder}"
	fi
	getNonFreeToNonFreeFirmware
}
upgradeToSid() {
...
upgradeSourcesList() {
	...
	else 															echo -e "\\tFile /etc/debian_version doesn't exists"
																	exit 1
	fi
}
upgradeDebianDist() { #if command -v sudo &> /dev/null; then
	if ! env | grep XDG_SESSION_TYPE=tty; then #check tty env
		...
		read -rp "continuer (y/N)" -n 1 sConfirmUpg
	else
		sConfirmUpg="y"
	fi
	if [[ "${sConfirmUpg,,}" = "y" ]]; then suExecCommandNoPreserveEnv "apt-get autoremove && apt-get update && apt-get upgrade && apt-get full-upgrade && apt-get dist-upgrade && apt-get autoremove"; fi
}
main() { #1st run recommended to update old distro 
	upgradeDebianDist
	upgradeSourcesList
