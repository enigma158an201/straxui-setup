...
getNewHostname() { if sHardwareModel=$(getProductName) && sOsId=$(getOsRelease); then echo "${sHardwareModel,,}-${sOsId}"; else return 1; fi; }
updateHostname() {
...
updateHosts() {
	if (grep -w "${sOldHostname}" "${sHostsFile}" &>/dev/null && ! grep -w "${sNewHostname}" "${sHostsFile}" &>/dev/null); then
