...
if [[ "$#" -ne 3 ]]; then echo "Usage: $0 </path/to/binary_on_disk> </path/to/file.tar.gz> <binary_in_tar.gz>"; exit 1; fi
...
main() {																# Extract the binary file from the tar.gz archive to a temporary directory
	...
	if cmp -s "${temp_dir}/${binary_in_tar}" "${binary_on_disk}"; then 	echo "Binary files are the same."	# Compare the binary files using the `cmp` command & Check the exit status to determine if the files are the same
	else 																echo "Binary files are different."
	fi
