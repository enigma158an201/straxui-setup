...
installNewSourcesList() {
	...
	if [[ -f "${sSourcesListDst}" ]]; then
		...
		if [[ "${sResultDiff}" = "" ]]; then bDiffApt="false"; else bDiffApt="true"; fi 	
	fi
	...
	if [[ "${confirmOverwriteRepo^^}" = "O" ]] || [[ "${bDiffApt}" = "true" ]]; then
		...
		if [[ -f "${sSourcesListDst}" ]]; then 	suExecCommand mv -i "${sSourcesListDst}" "${sSourcesListBak}"; fi
		...
		echo -e "\t--> abandon de l'installation de la version suivante pour ${sSourcesListSrc}"; exit 1
	fi
